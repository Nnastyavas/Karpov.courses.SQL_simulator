/*
Итак, мы выяснили, что первая рекламная кампания позволяет привлекать более качественных лидов. Но причина этих различий пока не совсем ясна. 
...
SELECT
	ads_campaign,
	round(avg(orders_avg),
	2) AS avg_check
FROM
	(
	SELECT
		ads_campaign,
		avg(order_price) AS orders_avg,
		user_id
	FROM
		(
		SELECT
